% Emma/Phil, 25th March 2023     First full analysis
% Emma/Phil, 22nd November 2025  Error analysis added
%
...
% - The species are in general isotopes of elements found in the ground "rock" (I guess), or sometimes functions of these
...
% - (you can say you're using a "deep learning" algorithm, and make reviewers squirm (-;)
...
% - Split the whole sample into nCV subsets at random (we need to think a bit about a sensible value for nCV)
...
% - Because we split (correct term == partition) the data in three at random, the answers we get will depend to some extent
...
%   This is EXACTLY what we want (I hope!!!)
%
% Other stuff to talk about when we get a chance
% - How do splines, penalised B-splines work? Basic maths. Pros and cons
...
% Set up main "fiddle factors" for the analysis (we need to check results are not too sensitive to these choices)
Anl.Dgn=0; %diagnosis mode (generate extra diagnostic plots to check things are working)
Anl.nRpl=1; %number of replicates (typically 100, or we can go mad later if you really want to)
Anl.nCV=10; %number of cross-validation subgroups (minimum number of observations per rift is 41, there as NaNs too, so nCV=5 feels OK)
Anl.nErr=1; %number of error analysis resamples to use
...
if 1; %from XL is you want
    %Dat=VlcGetDat('data_phil_spline_longlat_dist.xlsx',Anl.Dgn); %no error information
    %save Dat0 Dat0;
    Dat0=VlcGetDatErr('data_phil_spline_longlat_dist_update_Ce_Pb_100km_Pb_uncertainities.xlsx',Anl.Dgn); %error information
    save Dat0 Dat0;
...
% Loop of resample of data with error
for iE=1:Anl.nErr;

    fprintf(1,'Error resample %g of %g\n',iE,Anl.nErr);

    % Make sure you know which resample you're using to keep track of things
    Anl.ErrInd=iE;

    % Resample the data for analysis
    Dat=VlcRspDat(Dat0); %generate a local copy of the data for this analysis
    ...
    %VclSmm(Dat,Anl); %useful for spline model only
    VclSmm_S(Dat,Anl); %useful to compare spline and linear regression
    ...
    save(sprintf('Output/Anl_E%g',Anl.ErrInd),'Anl');

end; %loop over iE, the resample index
