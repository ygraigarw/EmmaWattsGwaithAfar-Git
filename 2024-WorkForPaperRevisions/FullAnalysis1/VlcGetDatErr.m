function Dat=VlcGetDatErr(FilNam,Dgn);
...
%you don't need distances to lake N(:,4) because we're going to calculate them, but use as a check
%Dat.X=N(:,5:end); %data matrix, basically the core of our analysis
Dat.X=N(:,5:18); %data matrix, basically the core of our analysis
[Dat.n,Dat.p]=size(Dat.X); %size of input matrix (this should be 290 x 14)

% Enter the error data; this number is interpreted as 2 standard deviations of the measurement noise
Dat.E=N(:,19:32); %error matrix
...
for i=1:Dat.n;
    Dat.Dst.C1(i,1)=pSphDst(Dat.LngLtt(i,1),Dat.LngLtt(i,2),Dat.C1.LngLtt(1),Dat.C1.LngLtt(2))*1e-3; %spherical distance in km
    for j=1:3;
        Dat.Dst.C3(i,j)=pSphDst(Dat.LngLtt(i,1),Dat.LngLtt(i,2),Dat.C3.LngLtt(j,1),Dat.C3.LngLtt(j,2))*1e-3;
    end;
end;
...
for j=1:3;
    plot(Dat.LngLtt(Dat.PlmInd==j,1),Dat.LngLtt(Dat.PlmInd==j,2),'color',pClr(j),'marker','o','linestyle','none');
end;
for j=1:3;
...
for j=1:3;
    plot(Dat.LngLtt(Dat.RftInd==j,1),Dat.LngLtt(Dat.RftInd==j,2),'color',pClr(j),'marker','o','linestyle','none');
end;
for j=1:3;
    plot(Dat.C3.LngLtt(j,1),Dat.C3.LngLtt(j,2),'marker','.','markersize',40,'color','k');
end;
...
if Dgn==1;
    ...
    figure(1); clf; hold on;
    for j=1:3;
        plot(Dat.Dst.C3(:,j),'color',pClr(j));
    end;
    for j=1:3;
        for k=1:3;
            plot(find(Dat.PlmInd==j & Dat.RftInd==k),Dat.Dst.C3(Dat.PlmInd==j & Dat.RftInd==k,j),'color',pClr(k),'marker','o','linestyle','none');
        end;
    end;
    pDfl; pAxsLmt;
